...
public class PMTableHierarchicalContainer extends PMTableIndexedContainer implements Container.Hierarchical {
	...
	protected void fireItemsAdded(Collection<Object> itemIds) {
		// adding an empty removed list to allow handling of collapsed items
		fireItemSetChange(new PMTableIndexedContainer.ItemSetChangeEvent(this, itemIds, new HashSet<Object>()));
	}

	@Override
	protected void fireItemAdded(Object itemId) {
		HashSet<Object> itemIds = new HashSet<Object>(1);
		itemIds.add(itemId);
		fireItemSetChange(new PMTableIndexedContainer.ItemSetChangeEvent(this, itemIds, new HashSet<Object>()));
	...
	/*
	 * Gets the ID of the parent of the specified Item. Don't add a JavaDoc comment here, we use the default documentation
	...
	/**
	 * <p>
	 * Sets the given Item's capability to have children. If the Item identified with the itemId already has children and
	 * the areChildrenAllowed is false this method fails and <code>false</code> is returned; the children must be first
	 * explicitly removed with {@link #setParent(Object itemId, Object newParentId)} or
	...
	public boolean setParent(Object itemId, Object newParentId) {
		...
		if (newParentId == null) {
			...
			fillChildrenIDsRecursive(itemId, ids);
			fireItemsAdded(ids);
	...
	private void fillChildrenIDsRecursive(Object id, Set<Object> ids) {
		ids.add(id);
		Collection<?> children = getChildren(id);
		if (children != null)
			for (Object i : children) {
				fillChildrenIDsRecursive(i, ids);
			}
	}
	...
	public Object addItem() {
		return super.addItem(); // It will call addItem(Object itemID)
	}
	 ...
	 * 
	 * @return true if all parents for items that match the filter are included when filtering, false if only the matching
	 *         items are included
	 */
	...
	/**
	 * Scans the itemId and all its children for which items should be included when filtering. All items which passes the
	 * filters are included. Additionally all items that have a child node that should be included are also themselves
	 * included.
	 * 
	 * @param itemId
	 * @param includedItems
