...
public class PMTableWidget extends FlowPanel implements HasWidgets, ScrollHandler, VHasDropHandler, FocusHandler,
	...
	/**
	 * The default multi select mode where simple left clicks only selects one item, CTRL+left click selects multiple
	 * items and SHIFT-left click selects a range of items.
	 */
	private static final int MULTISELECT_MODE_DEFAULT = 0;

	/**
	 * The simple multiselect mode is what the table used to have before ctrl/shift selections were added. That is that
	 * when this is set clicking on an item selects/deselects the item and no ctrl/shift selections are available.
	 */
	...
	/*
	 * When scrolling and selecting at the same time, the selections are not in sync with the server while retrieving new
	 * rows (until key is released).
	 */
	 ...
	 * For internal use only. May be removed or replaced in the future.
	 */
	public PMTableWidget.PMTableWidgetBody.PMTableWidgetRow focusedRow;
	 ...
	 * For internal use only. May be removed or replaced in the future.
	 */
	public PMTableWidget.PMTableWidgetBody.PMTableWidgetRow selectionRangeStart;
	...
	private static final boolean hasNativeTouchScrolling = BrowserInfo.get().isTouchDevice()
			&& !BrowserInfo.get().requiresTouchScrollDelegate();

	private Set<String> noncollapsibleColumns;

	/**
	 * The last known row height used to preserve the height of a table with custom row heights and a fixed page length
	 * after removing the last row from the table.
	 * 
	 * A new PMTableWidgetBody instance is created every time the number of rows changes causing
	 * {@link PMTableWidgetBody#rowHeight} to be discarded and the height recalculated by
	 * {@link PMTableWidgetBody#getRowHeight(boolean)} to avoid some rounding problems, e.g. round(2 * 19.8) / 2 = 20 but
	 * round(3 * 19.8) / 3 = 19.66.
	 */
	private double lastKnownRowHeight = Double.NaN;

	/**
	 * Remember scroll position when getting detached to properly scroll back to the location that there is data for if
	...
	private class SelectionRange {
		private PMTableWidgetRow startRow;
		private final int length;

		/**
		 * Constuctor.
		 */
		public SelectionRange(PMTableWidgetRow row1, PMTableWidgetRow row2) {
			PMTableWidgetRow endRow;
		...
		public SelectionRange(PMTableWidgetRow row, int length) {
			startRow = row;
			this.length = length;
		}
		...
		private boolean inRange(PMTableWidgetRow row) {
			return row.getIndex() >= startRow.getIndex() && row.getIndex() < startRow.getIndex() + length;
		}

		public Collection<SelectionRange> split(PMTableWidgetRow row) {
		...
		public void onKeyUp(KeyUpEvent keyUpEvent) {
			...
			if (!isFocusable()) {
				...
				if (keyCode == getNavigationDownKey() || keyCode == getNavigationUpKey()) {
					/*
					 * in multiselect mode the server may still have value from previous page. Clear it unless doing
	...
	/** For internal use only. May be removed or replaced in the future. */
	public PMTableWidgetBody scrollBody;
	...
	/**
	 * This map contains captions and icon urls for actions like: * "33_c" -> "Edit" * "33_i" -> "http://dom.com/edit.png"
	 */
	...
	/**
	 * Read from the "recalcWidths" -attribute. When it is true, the table will recalculate the widths for columns -
	 * desirable in some cases. For #1983, marked experimental. See also variable <code>refreshContentWidths</code> in
	 * method {@link TableHead#updateCellsFromUIDL(UIDL)}.
	 * <p>
	 * For internal use only. May be removed or replaced in the future.
	 */
	...
	/**
	 * Values (serverCacheFirst+serverCacheLast) sent by server that tells which rows (indexes) are in the server side
	 * cache (page buffer). -1 means unknown. The server side cache row MUST MATCH the client side cache rows.
	 * 
	 * If the client side cache contains additional rows with e.g. buttons, it will cause out of sync when such a button
	 * is pressed.
	 * 
	 * If the server side cache contains additional rows with e.g. buttons, scrolling in the client will cause empty
	 * buttons to be rendered (cached=true request for non-existing components)
	 * 
	 * For internal use only. May be removed or replaced in the future.
	 */
	...
	/**
	 * In several cases TreeTable depends on the scrollBody.lastRendered being 'out of sync' while the update is being
	 * done. In those cases the sanity check must be performed afterwards.
	 */
	...
	public PMTableWidget() {
		...
		/*
		 * Firefox auto-repeat works correctly only if we use a key press handler, other browsers handle it correctly when
		 * using a key down handler
		 */
		if (BrowserInfo.get().isGecko()) {
	...
	/**
	 * Fires a column resize event which sends the resize information to the server.
	 * 
	 * @param columnId
	...
	/**
	 * Non-immediate variable update of column widths for a collection of columns.
	 * 
	 * @param columns
	 ...
	 * 
	 * @return Returns true if succeeded, else false if the selection could not be move downwards
	 */
	private boolean moveFocusDown(int offset) {
		if (isSelectable()) {
			if (focusedRow == null && scrollBody.iterator().hasNext()) {
				...
				// ??
				return setRowFocus((PMTableWidgetRow) scrollBody.iterator().next());
			} else {
				PMTableWidgetRow next = getNextRow(focusedRow, offset);
				if (next != null) {
	 ...
	 * 
	 * @return Returns true if succeeded, else false if the selection could not be move upwards
	 * 
	 */
	private boolean moveFocusUp(int offset) {
		if (isSelectable()) {
			if (focusedRow == null && scrollBody.iterator().hasNext()) {
				...
				PMTableWidgetRow prev = getPreviousRow(focusedRow, offset);
				if (prev != null) {
	...
	protected void sendSelectedRows(boolean immediately) {
		...
		if (isMultiSelectModeDefault()) {
			...
			for (Iterator<String> iterator = selectedRowKeys.iterator(); iterator.hasNext();) {
				...
				PMTableWidgetRow renderedRowByKey = getRenderedRowByKey(key);
		...
		client.updateVariable(paintableId, "selected", selectedRowKeys.toArray(new String[selectedRowKeys.size()]),
				immediately);

	}

	/**
	 * Get the key that moves the selection head upwards. By default it is the up arrow key but by overriding this you can
	...
	/**
	 * Get the key that moves the selection head downwards. By default it is the down arrow key but by overriding this you
	 * can change the key to whatever you want.
	 * 
	 * @return The keycode of the key
	 */
	...
	/**
	 * Get the key that scrolls to the left in the table. By default it is the left arrow key but by overriding this you
	 * can change the key to whatever you want.
	 * 
	 * @return The keycode of the key
	 */
	...
	/**
	 * Get the key that scroll to the right on the table. By default it is the right arrow key but by overriding this you
	 * can change the key to whatever you want.
	 * 
	 * @return The keycode of the key
	 */
	...
	/**
	 * Get the key that selects an item in the table. By default it is the space bar key but by overriding this you can
	 * change the key to whatever you want.
	 * 
	 * @return
	 */
	...
	/**
	 * Get the key the moves the selection one page up in the table. By default this is the Page Up key but by overriding
	 * this you can change the key to whatever you want.
	 * 
	 * @return
	 */
	...
	/**
	 * Get the key the moves the selection one page down in the table. By default this is the Page Down key but by
	 * overriding this you can change the key to whatever you want.
	 * 
	 * @return
	 */
	...
	/**
	 * Get the key the moves the selection to the beginning of the table. By default this is the Home key but by
	 * overriding this you can change the key to whatever you want.
	 * 
	 * @return
	 */
	...
	/**
	 * Get the key the moves the selection to the end of the table. By default this is the End key but by overriding this
		...
		if (uidl.hasVariable("selected")) {
			...
			if (scrollBody != null) {
				...
				while (iterator.hasNext()) {
					/*
					 * Make the focus reflect to the server side state unless we are currently selecting multiple rows with
					 * keyboard.
					 */
					PMTableWidgetRow row = (PMTableWidgetRow) iterator.next();
					boolean selected = selectedKeys.contains(row.getKey());
					if (!selected && unSyncedselectionsBeforeRowFetch != null
							&& unSyncedselectionsBeforeRowFetch.contains(row.getKey())) {
						selected = true;
	...
	public void updateScrollTop(UIDL uidl) {
		...
		if (scrollBodyPanel.getScrollPosition() != scrollTop) {
			/*
	...
	public void updateSelectionProperties(UIDL uidl, AbstractComponentState state, boolean readOnly) {
		setMultiSelectMode(uidl.hasAttribute("multiselectmode") ? uidl.getIntAttribute("multiselectmode")
	...
	/**
	 * Returns the extra space that is given to the header column when column width is determined by header text.
	 * 
	...
	/**
	 * This method exists for the needs of {@link VTreeTable} only. Not part of the official API, <b>extend at your own
	 * risk</b>. May be removed or replaced in the future.
	 * 
	...
	public void focusRowFromBody() {
		...
		if (selectedRowKeys.size() == 1) {
			...
			if (selectedRowKey != null) {
				PMTableWidgetRow renderedRow = getRenderedRowByKey(selectedRowKey);
				setRowFocus(renderedRow);
			}
		} else {
			// TODO raffael
			// multiselect mode
			setRowFocus(scrollBody.getRowByRowIndex(0));
		}
	}

	protected PMTableWidgetBody createScrollBody() {
		return new PMTableWidgetBody();
	}
	...
	public void updateBody(UIDL uidl, int reqRows) {
		...
		scrollBody.renderRows(uidl, reqRows);
	...
	public void updateRowsInBody(UIDL partialRowUpdates) {
		...
		scrollBody.aligns = tHead.getColumnAlignments();
		final Iterator<?> it = partialRowUpdates.getChildIterator();
		while (it.hasNext()) {
	...
	public void addAndRemoveRows(UIDL partialRowAdditions) {
		...
		if (partialRowAdditions.hasAttribute("drows")) {
			String[] toRemove = partialRowAdditions.getStringArrayAttribute("drows");
			List<Integer> ixs = new ArrayList<Integer>();
			for (int i = 0; i < toRemove.length; i++) {
				String keyS = toRemove[i];
				PMTableWidgetRow row = getRenderedRowByKey(keyS);
				ixs.add(row.getIndex());
			}
	 ...
	 *          the UIDL containing row updates.
	 */
	public void reorderRowsInBody(String[] order) {
		if (order.length == 0)
			return;
		if (order.length != scrollBody.size())
			throw new RuntimeException("can not reorder. wrong key size");

		HashMap<String, PMTableWidgetRow> rows = new HashMap<String, PMTableWidget.PMTableWidgetBody.PMTableWidgetRow>();
		for (Widget w : scrollBody.renderedRows) {
			PMTableWidgetRow row = (PMTableWidgetRow) w;
			rows.put(row.getKey(), row);
		}
		scrollBody.renderedRows.clear();
		for (String k : order) {
			PMTableWidgetRow row = rows.get(k);
			scrollBody.addRow(row, false);
		}
	}
	...
	private boolean isMultiSelectModeSimple() {
		return selectMode == SelectMode.MULTI && multiselectmode == MULTISELECT_MODE_SIMPLE;
	}
	...
	private boolean isMultiSelectModeDefault() {
		return selectMode == SelectMode.MULTI && multiselectmode == MULTISELECT_MODE_DEFAULT;
	}
	...
	/**
	 * Note: not part of the official API, extend at your own risk. May be removed or replaced in the future.
	 * 
	 ...
	 * @param w
	 *          new width (may be subject to modifications if doesn't meet minimum requirements)
	 ...
	 * @return
	 */
	public PMTableWidgetRow getRenderedRowByKey(String key) {
		if (scrollBody != null) {
			final Iterator<Widget> it = scrollBody.iterator();
			PMTableWidgetRow r = null;
			while (it.hasNext()) {
				r = (PMTableWidgetRow) it.next();
	 ...
	 */
	private PMTableWidgetRow getNextRow(PMTableWidgetRow row, int offset) {
		final Iterator<Widget> it = scrollBody.iterator();
		PMTableWidgetRow r = null;
		while (it.hasNext()) {
			r = (PMTableWidgetRow) it.next();
	 ...
	 */
	private PMTableWidgetRow getPreviousRow(PMTableWidgetRow row, int offset) {
		final Iterator<Widget> it = scrollBody.iterator();
		final Iterator<Widget> offsetIt = scrollBody.iterator();
		PMTableWidgetRow r = null;
		PMTableWidgetRow prev = null;
		while (it.hasNext()) {
			r = (PMTableWidgetRow) it.next();
			if (offset < 0) {
				prev = (PMTableWidgetRow) offsetIt.next();
			}
			if (r == row) {
				return prev;
			}
	...
	protected void reOrderColumn(String columnKey, int newIndex) {
		...
		/*
		 * Build new columnOrder and update it to server Note that columnOrder also contains collapsed columns so we cannot
		 * directly build it from cells vector Loop the old columnOrder and append in order to new array unless on moved
		 * columnKey. On new index also put the moved key i == index on columnOrder, j == index on newOrder
		 */
		...
		if (client.hasEventListeners(this, PMTableConstants.COLUMN_REORDER_EVENT_ID)) {
	...
	/**
	 * Run only once when component is attached and received its initial content. This function:
	 * 
	...
	public void sizeInit() {
		...
		/*
		 * We will use browsers table rendering algorithm to find proper column widths. If content and header take less
		 * space than available, we will divide extra space relatively to each column which has not width set.
		 * 
		...
		if (availW > total) {
			...
			if (hierarchyHeaderWithExpandRatio != null) {
				/*
				 * add the indent's space back to ensure each column gets an even share according to the expand ratios (note: if
				 * the allocated space isn't enough for the hierarchy column it shall be treated like a defined width column and
				 * the indent space gets removed from the extra space again)
				 */
			...
			if (extraSpace == 1) {
				...
				if (hierarchyHeaderWithExpandRatio != null) {
					...
					final int newSpace = Math
							.round((extraSpace * (hierarchyHeaderWithExpandRatio.getExpandRatio() / expandRatioDivider)));
			...
			if (extraSpace > 0 && checksum != availW) {
				/*
				 * There might be in some cases a rounding error of 1px when extra space is divided so if there is one then we
				 * give the first undefined column 1 more pixel
				 */
				headCells = tHead.iterator();
				i = 0;
				while (headCells.hasNext()) {
		...
		/*
		 * Fix "natural" height if height is not set. This must be after width fixing so the components' widths have been
		 * adjusted.
		 */
		if (isDynamicHeight()) {
			/*
			 * We must force an update of the row height as this point as it might have been (incorrectly) calculated earlier
			 */

			/*
			 * TreeTable updates stuff in a funky order, so we must set the height as zero here before doing the real update
			 * to make it realize that there is no content,
			 */
			scrollBody.setHeight("0px");

			int bodyHeight;
			/*
			 * A hack to support variable height rows when paging is off. Generally this is not supported by scrolltable. We
			 * want to show all rows so the bodyHeight should be equal to the table height.
			 */
			// int bodyHeight = scrollBody.getOffsetHeight();
			bodyHeight = Util.getRequiredHeight(scrollBody.table);
	...
	protected boolean willHaveScrollbars() {
		if (isDynamicHeight())
			return false;
		if (scrollBody == null || scrollBody.tBodyElement == null)
			return false;
		int bodyH = scrollBody.tBodyElement.getOffsetHeight();
		// int totalH = getOffsetHeight();
		// if (showColHeaders)
		// totalH = totalH - tHead.getOffsetHeight();
		// if (showColFooters)
		// totalH = totalH - tFoot.getOffsetHeight();

		return bodyH >= containerHeight;
	}
	...
	public boolean isScrollPositionVisible() {
		return scrollPositionElement != null
				&& !scrollPositionElement.getStyle().getDisplay().equals(Display.NONE.toString());
	}
	...
	public class HeaderCell extends Widget {
		...
		/**
		 * Makes room for the sorting indicator in case the column that the header cell belongs to is sorted. This is done
		 * by resizing the width of the caption container element by the correct amount
		 */
		...
		/**
		 * Sets width to the header cell. This width should not include any possible indent modifications that are present
		 * in {@link PMTableWidgetBody#getMaxIndent()}.
		 * 
		 * @param w
		...
		public void setWidth(int w, boolean ensureDefinedWidth) {
			...
			if (w == -1) {
				...
				/*
				 * if we already have tBody, set the header width properly, if not defer it. IE will fail with complex float in
				 * table header unless TD width is not explicitly set.
				 */
				if (scrollBody != null) {
		 ...
		 * 
		 * Returns the pixels width of the header cell. This includes the indent, if applicable.
		 * 
		 ...
		 * This method exists for the needs of {@link VTreeTable} only.
		 * 
		 * @return <code>true</code> if this is hierarcyColumn's header cell, <code>false</code> otherwise
		 */
		private boolean isHierarchyColumn() {
			int hierarchyColumnIndex = getHierarchyColumnIndex();
			return hierarchyColumnIndex >= 0 && tHead.visibleCells.indexOf(this) == hierarchyColumnIndex;
		}
		...
		private void setSorted(boolean sorted) {
			this.sorted = sorted;
			updateStyleNames(PMTableWidget.this.getStylePrimaryName());
		}
		...
		public void onBrowserEvent(Event event) {
			if (enabled && event != null) {
				if (isResizing || event.getEventTarget().cast() == colResizeWidget) {
					...
					/*
					 * Ensure focus before handling caption event. Otherwise variables changed from caption event may be before
					 * variables from other components that fire variables when they lose focus.
					 */
					...
					if (event.getTypeInt() == Event.ONCONTEXTMENU
							&& !client.hasEventListeners(PMTableWidget.this, PMTableConstants.HEADER_CLICK_EVENT_ID)) {
		...
		private void fireHeaderClickedEvent(Event event) {
			if (client.hasEventListeners(PMTableWidget.this, PMTableConstants.HEADER_CLICK_EVENT_ID)) {
				...
				if (!moved) {
					...
					if (sortable && Util.isTouchEventOrLeftMouseButton(event)) {
						if (sortColumn.equals(cid)) {
							// just toggle order
							client.updateVariable(paintableId, "sortascending", !sortAscending, true);
						} else {
							// set table sorted by this column
							client.updateVariable(paintableId, "sortcolumn", cid, true);
						}
		 ...
		 * 
		 * @param includeIndent
		 *          - width should include hierarchy column indent if applicable (VTreeTable only)
		 * @param includeCellExtraWidth
		 *          - width should include paddings etc.
		 * @return
		 */
		private int getMinWidth(boolean includeIndent, boolean includeCellExtraWidth) {
	...
	public class TableHead extends Panel implements ActionOwner {
		...
		public void resizeCaptionContainer(HeaderCell cell) {
			...
			if (cell == lastcell && columnSelectorOffset > 0 && !hasVerticalScrollbar()) {
				...
				for (Widget w : visibleCells) {
					...
					if (scrollBody != null && visibleCells.indexOf(w) == getHierarchyColumnIndex()
				...
				if (columnTotalWidth >= divOffset - columnSelectorOffset) {
					/*
					 * Ensure column caption is visible when placed under the column selector widget by shifting and resizing the
					 * caption.
					 */
					int offset = 0;
					...
					if (diff < columnSelectorOffset && diff > 0) {
						/*
						 * If the difference is less than the column selectors width then just offset by the difference
						 */
		...
		public void updateCellsFromUIDL(UIDL uidl) {
			...
			boolean refreshContentWidths = initializedAndAttached && hadScrollBars != willHaveScrollbars();
			while (it.hasNext()) {
		...
		private void focusSlot(int index) {
			...
			if (index > 0) {
				...
				child.setClassName(PMTableWidget.this.getStylePrimaryName() + "-resizer");
				child.addClassName(PMTableWidget.this.getStylePrimaryName() + "-focus-slot-right");
			} else {
				Element child = tr.getChild(index).getFirstChild().cast();
				child.setClassName(PMTableWidget.this.getStylePrimaryName() + "-resizer");
				child.addClassName(PMTableWidget.this.getStylePrimaryName() + "-focus-slot-left");
			}
			focusedSlot = index;
		}
		...
		class VisibleColumnAction extends Action {
			...
			private PMTableWidgetRow currentlyFocusedRow;

			public VisibleColumnAction(String colKey) {
				super(PMTableWidget.TableHead.this);
				this.colKey = colKey;
			...
			public void execute() {
				...
				client.updateVariable(paintableId, "collapsedcolumns",
						collapsedColumns.toArray(new String[collapsedColumns.size()]), true);
		...
		/**
		 * Disables the automatic calculation of all column widths by forcing the widths to be "defined" thus turning off
		 * expand ratios and such.
		 */
	...
	public class FooterCell extends Widget {
		...
		protected void updateStyleNames(String primaryStyleName) {
			captionContainer.setClassName(primaryStyleName + "-footer-container");
		}
		...
		/**
		 * Sets the width of the cell. This width should not include any possible indent modifications that are present in
		 * {@link PMTableWidgetBody#getMaxIndent()}.
		 * 
		 * @param w
		...
		public void setWidth(int w, boolean ensureDefinedWidth) {
			...
			if (w == -1) {
				...
				/*
				 * Reduce width with one pixel for the right border since the footers does not have any spacers between them.
				 */
		...
		public void onBrowserEvent(Event event) {
			if (enabled && event != null) {
				...
				boolean stopPropagation = true;
				if (event.getTypeInt() == Event.ONCONTEXTMENU
						&& !client.hasEventListeners(PMTableWidget.this, PMTableConstants.FOOTER_CLICK_EVENT_ID)) {
		...
		/**
		 * Fires a footer click event after the user has clicked a column footer cell
		 * 
		 * @param event
		 *          The click event
		 */
		private void fireFooterClickedEvent(Event event) {
			if (client.hasEventListeners(PMTableWidget.this, PMTableConstants.FOOTER_CLICK_EVENT_ID)) {
				MouseEventDetails details = MouseEventDetailsBuilder.buildMouseEventDetails(event);
	...
	/**
	 * This Panel can only contain PMTableWidgetRow type of widgets. This "simulates" very large table, keeping spacers
	 * which take room of unrendered rows.
	 * 
	 */
	public class PMTableWidgetBody extends com.google.gwt.user.client.ui.Panel { // FIXME No idea why my maven compiler
		...
		protected PMTableWidgetBody() {
			constructDOM();
			setElement(container);
		}

		public PMTableWidgetRow getRowByRowIndex(int indexInTable) {
			int internalIndex = indexInTable;
			if (internalIndex >= 0 && internalIndex < size()) {
				return (PMTableWidgetRow) renderedRows.get(internalIndex);
			} else {
				return null;
			}
		}
		...
		public void renderInitialRows(UIDL rowData, int rows) {
			final Iterator<?> it = rowData.getChildIterator();
			aligns = tHead.getColumnAlignments();
			while (it.hasNext()) {
				final PMTableWidgetRow row = createRow((UIDL) it.next(), aligns);
				addRow(row, true);
			}
		}

		public void renderRows(UIDL rowData, int rows) {
			// FIXME REVIEW
			aligns = tHead.getColumnAlignments();
			final Iterator<?> it = rowData.getChildIterator();
			...
			while (it.hasNext()) {
				addRow(prepareRow((UIDL) it.next()), true);
			}
			setContainerHeight();
		}
		 ...
		 */
		protected void insertRows(UIDL rowData) {
			aligns = tHead.getColumnAlignments();
			Iterator<?> it = rowData.getChildIterator();
			ArrayList<Integer> ixs = new ArrayList<Integer>();
			while (it.hasNext()) {
				int key = ((UIDL) it.next()).getIntAttribute("key");
				PMTableWidgetRow oldRow = getRenderedRowByKey(Integer.toString(key));
				if (oldRow != null)
					ixs.add(oldRow.getIndex());
			}
			Collections.sort(ixs);
			Collections.reverse(ixs);
			for (int i : ixs)
			...
			while (it.hasNext()) {
				PMTableWidgetRow row = prepareRow((UIDL) it.next());
				insertRowAt(row, row.getIndex());
				row.getElement().getStyle().setProperty("visibility", "");
			}
		}

		/**
		 * This method is used to instantiate new rows for this table. It automatically sets correct widths to rows cells
		 * and assigns correct client reference for child widgets.
		 * 
		 * This method can be called only after table has been initialized
		 * 
		 * @param uidl
		 */
		private PMTableWidgetRow prepareRow(UIDL uidl) {
			final PMTableWidgetRow row = createRow(uidl, aligns);
			row.initCellWidths();
			return row;
		}

		protected PMTableWidgetRow createRow(UIDL uidl, char[] aligns2) {
			if (uidl.hasAttribute("gen_html")) {
				// This is a generated row.
				return new PMTableWidgetGeneratedRow(uidl, aligns2);
			}
			return new PMTableWidgetRow(uidl, aligns2);
		}

		private void addRow(PMTableWidgetRow row, boolean adopt) {
			row.setIndex(size());
			if (row.isSelected()) {
		...
		private void insertRowAt(PMTableWidgetRow row, int index) {
			row.setIndex(index);
			if (row.isSelected()) {
				row.addStyleName("v-selected");
			}
			if (index > 0) {
				PMTableWidgetRow sibling = getRowByRowIndex(index - 1);
				tBodyElement.insertAfter(row.getElement(), sibling.getElement());
			} else {
				tBodyElement.insertFirst(row.getElement());
			}
			adopt(row);
		...
		protected void reindexFrom(int start) {
			for (int ix = start; ix < size(); ix++) {
				PMTableWidgetRow r = (PMTableWidgetRow) renderedRows.get(ix);
				r.setIndex(ix);
			}
		}

		private void unlinkRow(int index) {
			final PMTableWidgetRow toBeRemoved = (PMTableWidgetRow) renderedRows.get(index);
		 ...
		 * 
		 * Due IE limitation, we must set the width to a wrapper elements inside table cells (with overflow hidden, which
		 * does not work on td elements).
		 * 
		 * To get this work properly crossplatform, we will also set the width of td.
		 * 
		 * @param colIndex
		 * @param w
		 */
		public void setColWidth(int colIndex, int w) {
			for (Widget row : renderedRows) {
				((PMTableWidgetRow) row).setCellWidth(colIndex, w);
			}
		}
		...
		/**
		 * This method exists for the needs of {@link VTreeTable} only. May be removed or replaced in the future.</br> </br>
		 * Returns the maximum indent of the hierarcyColumn, if applicable.
		 * 
		 * @see {@link PMTableWidget#getHierarchyColumnIndex()}
		 * 
		...
		private void detectExtrawidth() {
			...
			if (rows.getLength() == 0) {
				...
				PMTableWidgetRow scrollTableRow = new PMTableWidgetRow();
				scrollTableRow.updateStyleNames(PMTableWidget.this.getStylePrimaryName());
				...
				if (firstTD == null) {
					...
					// for measuring
					noCells = true;
					PMTableWidgetRow next = (PMTableWidgetRow) iterator().next();
		...
		public void moveCol(int oldIndex, int newIndex) {
			...
			while (rows.hasNext()) {
				final PMTableWidgetRow row = (PMTableWidgetRow) rows.next();
		...
		/**
		 * Restore row visibility which is set to "none" when the row is rendered (due a performance optimization).
		 */
		...
		public class PMTableWidgetRow extends Panel implements ActionOwner, ContextMenuOwner {
			...
			private PMTableWidgetRow(int rowKey) {
				...
				DOM.sinkEvents(getElement(), Event.MOUSEEVENTS | Event.TOUCHEVENTS | Event.ONDBLCLICK | Event.ONCONTEXTMENU
						| VTooltip.TOOLTIP_EVENTS);
			}

			public PMTableWidgetRow(UIDL uidl, char[] aligns) {
				this(uidl.getIntAttribute("key"));

				/*
				 * Rendering the rows as hidden improves Firefox and Safari performance drastically.
				 */
				getElement().getStyle().setProperty("visibility", "hidden");

				index = uidl.getIntAttribute("index");
			...
			protected void initCellWidths() {
				...
				for (int i = 0; i < cells; i++) {
					int w = PMTableWidget.this.getColWidth(getColKeyByIndex(i));
					if (w < 0) {
						w = 0;
					}
					setCellWidth(i, w);
			 ...
			 */
			public boolean isBefore(PMTableWidgetRow row1) {
				return getIndex() < row1.getIndex();
			}

			/**
			 * Sets the index of the row in the whole table. Currently used just to set even/odd classname
			 * 
			...
			protected void initCellWithText(String text, char align, String style, boolean textIsHTML, boolean sorted,
				...
				td.setClassName(PMTableWidget.this.getStylePrimaryName() + "-cell-content");

				if (style != null && !style.equals("")) {
					td.addClassName(PMTableWidget.this.getStylePrimaryName() + "-cell-content-" + style);
				}

				if (sorted) {
					td.addClassName(PMTableWidget.this.getStylePrimaryName() + "-cell-content-sorted");
				}
			...
			protected void updateCellStyleNames(TableCellElement td, String primaryStyleName) {
				...
				td.setClassName(className.replaceAll(oldPrimaryName, primaryStyleName));
			}

			public void addCell(UIDL rowUidl, Widget w, char align, String style, boolean sorted, String description) {
				final TableCellElement td = DOM.createTD().cast();
			...
			protected void initCellWithWidget(Widget w, char align, String style, boolean sorted, final TableCellElement td) {
				final Element container = DOM.createDiv();
				String className = PMTableWidget.this.getStylePrimaryName() + "-cell-content";
				if (style != null && !style.equals("")) {
					className += " " + PMTableWidget.this.getStylePrimaryName() + "-cell-content-" + style;
				}
				if (sorted) {
					className += " " + PMTableWidget.this.getStylePrimaryName() + "-cell-content-sorted";
				}
			...
			/**
			 * If there are registered click listeners, sends a click event and returns true. Otherwise, does nothing and
			 * returns false.
			 * 
			 * @param event
			 * @param targetTdOrTr
			...
			private boolean handleClickEvent(Event event, Element targetTdOrTr, boolean immediate) {
				if (!client.hasEventListeners(PMTableWidget.this, PMTableConstants.ITEM_CLICK_EVENT_ID)) {
			...
			public void onBrowserEvent(final Event event) {
				...
				if (enabled && !touchEventHandled) {
					...
					case Event.ONTOUCHCANCEL:
						if (touchStart != null) {
							/*
							 * Touch has not been handled as neither context or drag start, handle it as a click.
							 */
							Util.simulateClickFromTouchEvent(touchStart, this);
							touchStart = null;
						}
					...
					case Event.ONTOUCHMOVE:
						if (isSignificantMove(event)) {
							/*
							 * TODO figure out scroll delegate don't eat events if row is selected. Null check for active delegate is
							 * as a workaround.
							 */
						...
						if (dragmode != 0 || actionKeys != null) {
							new Timer() {
								...
								public void run() {
									...
									/*
									 * If there's a scroll delegate, check if we're actually scrolling and handle it. If no delegate, do
									 * nothing here and let the row handle potential drag'n'drop or context menu.
									 */
									if (activeScrollDelegate != null) {
										if (activeScrollDelegate.isMoved()) {
											/*
											 * Prevent the row from handling touch move/end events (the delegate handles those) and from doing
											 * drag'n'drop or opening a context menu.
											 */
											touchStart = null;
										} else {
											/*
											 * Scrolling hasn't started, so cancel delegate and let the row handle potential drag'n'drop or
											 * context menu.
											 */
			...
			private boolean isSignificantMove(Event event) {
				...
				/*
				 * TODO calculate based on real distance instead of separate axis checks
				 */
				Touch touch = event.getChangedTouches().get(0);
			...
			protected void startRowDrag(Event event, final int type, Element targetTdOrTr) {
				...
				transferable.setDragSource(ConnectorMap.get(client).getConnector(PMTableWidget.this));
			...
			/**
			 * Finds the TD that the event interacts with. Returns null if the target of the event should not be handled. If
			 * the event target is the row directly this method returns the TR element instead of the TD.
			 * 
			 * @param event
			 * @return TD or TR element that the event targets (the actual event target is this element or a child of it)
			 */
			...
			private Element getElementTdOrTr(Element element) {
				...
				if (widget != this) {
					/*
					 * This is a workaround to make Labels, read only TextFields and Embedded in a Table clickable (see #2688). It
					 * is really not a fix as it does not work with a custom read only components (not extending
					 * VLabel/VEmbedded).
					 */
			...
			public void showContextMenu(Event event) {
				if (enabled && actionKeys != null) {
					...
					int left = Util.getTouchOrMouseClientX(event) + Window.getScrollLeft();
					int top = Util.getTouchOrMouseClientY(event) + Window.getScrollTop();
			...
			/**
			 * Is called when a user clicks an item when holding SHIFT key down. This will select a new range from the last
			 * focused row
			 * 
			...
			private void toggleShiftSelection(boolean deselectPrevious) {
				...
				/*
				 * Ensures that we are in multiselect mode and that we have a previous selection which was not a deselection
				 */
				...
				PMTableWidgetRow endRow = this;
				PMTableWidgetRow startRow = selectionRangeStart;
				if (startRow == null) {
					startRow = focusedRow;
			...
			public Action[] getActions() {
				...
				for (int i = 0; i < actions.length; i++) {
					...
					final TreeAction a = new TreeAction(this, String.valueOf(rowKey), actionKey) {
						...
						public void execute() {
							super.execute();
							lazyRevertFocusToRow(PMTableWidgetRow.this);
						}
					};
		...
		protected class PMTableWidgetGeneratedRow extends PMTableWidgetRow {

			private boolean spanColumns;
			private boolean htmlContentAllowed;

			public PMTableWidgetGeneratedRow(UIDL uidl, char[] aligns) {
				super(uidl, aligns);
		 ...
		 * 
		 * TODO the current implementation simply always calls focus for the component. In case the Table at some point
		 * implements focus/blur listeners, this method needs to be evolved to conditionally call focus only if not
		 * currently focused.
		 */
	...
	/**
	 * Used in multiselect mode when the client side knows that all selections are in the next request.
	 */
	...
	public void updateWidth() {
		if (!isVisible()) {
			/*
			 * Do not update size when the table is hidden as all column widths will be set to zero and they won't be
			 * recalculated when the table is set visible again (until the size changes again)
			 */
			return;
		}
		...
		/*
		 * setting width may affect wheter the component has scrollbars -> needs scrolling or not
		 */
		setProperTabIndex();
	}
	...
	private final Timer lazyAdjustColumnWidths = new Timer() {
		/**
		 * Check for column widths, and available width, to see if we can fix column widths "optimally". Doing this lazily
		 * to avoid expensive calculation when resizing is not yet finished.
		 */
		...
		public void run() {
			...
			// TODO raffael
			if (extraSpace < 0) {
				if (scrollBody.size() == 0) {
					/*
					 * Too wide header combined with no rows in the table.
					 * 
					 * No horizontal scrollbars would be displayed because there's no rows that grows too wide causing the
					 * scrollBody container div to overflow. Must explicitely force a width to a scrollbar. (see #9187)
					 */
			...
			if (forceScrollBodyWidth > 0) {
				scrollBody.container.getStyle().setWidth(forceScrollBodyWidth, Unit.PX);
			} else {
			...
			if (hierarchyHeaderInNeedOfFurtherHandling != null && !hierarchyHeaderInNeedOfFurtherHandling.isDefinedWidth()) {
				// ensure the cell gets enough space for the indent
				int w = hierarchyHeaderInNeedOfFurtherHandling.getNaturalColumnWidth(hierarchyColumnIndex);
				int newSpace = Math.round(w + (float) extraSpace * (float) w / totalUndefinedNaturalWidths);
				if (newSpace >= hierarchyColumnIndent) {
					...
					expandRatioDivider += hierarchyHeaderInNeedOfFurtherHandling.getExpandRatio();
			...
			if (isDynamicHeight()) {
				...
				int bodyHeight = Util.getRequiredHeight(scrollBody.table);
	...
	private void forceRealignColumnHeaders() {
		if (BrowserInfo.get().isIE()) {
			/*
			 * IE does not fire onscroll event if scroll position is reverted to 0 due to the content element size growth.
			 * Ensure headers are in sync with content manually. Safe to use null event as we don't actually use the event
			 * object in listener.
			 */
			onScroll(null);
		}
	...
	private void setContainerHeight() {
		if (!isDynamicHeight()) {
			...
			/*
			 * Android 2.3 cannot measure the height of the inline-block properly, and will return the wrong offset height. So
			 * for android 2.3 we set the element to a block element while measuring and then restore it which yields the
			 * correct result. #11331
			 */
	...
	/** For internal use only. May be removed or replaced in the future. */
	public PMTableWidgetDropHandler dropHandler;
	...
	public void setHeight(String height) {
		if (height.length() == 0 && getElement().getStyle().getHeight().length() != 0) {
			/*
			 * Changing from defined to undefined size -> should do a size init to take page length into account again
			 */
			sizeNeedsInit = true;
		}
	...
	public void updateHeight() {
		...
		/*
		 * setting height may affect wheter the component has scrollbars -> needs scrolling or not
		 */
		setProperTabIndex();
	...
	/*
	 * Overridden due Table might not survive of visibility change (scroll pos lost). Example ITabPanel just set contained
	 * components invisible and back when changing tabs.
	 */
	...
	protected String buildCaptionHtmlSnippet(UIDL uidl) {
		...
		if (uidl.hasAttribute("icon")) {
			s = "<img src=\"" + Util.escapeAttribute(client.translateVaadinUri(uidl.getStringAttribute("icon")))
	...
	public void onScroll(ScrollEvent event) {
		 ...
		 * 
		 * FIXME initializedAndAttached should probably be renamed - its name doesn't seem to reflect its semantics.
		 * onDetach() doesn't set it to false, and changing that might break something else, so we need to check
	...
	@Override
	public PMTableWidgetDropHandler getDropHandler() {
		return dropHandler;
	}
	...
	public class PMTableWidgetDropHandler extends VAbstractDropHandler {
		...
		private void updateDropDetails(VDragEvent drag) {
			...
			drag.getDropDetails()
					.put("detail", dropDetails.dropLocation != null ? dropDetails.dropLocation.toString() : null);
		...
		private void deEmphasis() {
			UIObject.setStyleName(getElement(), PMTableWidget.this.getStylePrimaryName() + "-drag", false);
			...
			for (Widget w : scrollBody.renderedRows) {
				...
				if (lastEmphasized != null && row.rowKey == lastEmphasized.overkey) {
					...
					PMTableWidgetRow.setStyleName(row.getElement(), stylename, false);
					lastEmphasized = null;
					return;
				}
			}
		...
		/**
		 * TODO needs different drop modes ?? (on cells, on rows), now only supports rows
		 */
		private void emphasis(TableDDDetails details) {
			deEmphasis();
			UIObject.setStyleName(getElement(), PMTableWidget.this.getStylePrimaryName() + "-drag", true);
			...
			for (Widget w : scrollBody.renderedRows) {
				...
				if (details != null && details.overkey == row.rowKey) {
					...
					PMTableWidgetRow.setStyleName(row.getElement(), stylename, true);
		...
		public ComponentConnector getConnector() {
			return ConnectorMap.get(client).getConnector(PMTableWidget.this);
		}
	...
	protected PMTableWidgetRow getFocusedRow() {
		return focusedRow;
	}
	 ...
	 */
	public boolean setRowFocus(PMTableWidgetRow row) {
	 ...
	 */
	private void ensureRowIsVisible(PMTableWidgetRow row) {
		if (BrowserInfo.get().isTouchDevice()) {
	...
	protected boolean handleNavigation(int keycode, boolean ctrl, boolean shift) {
		...
		if (keycode == getNavigationPageDownKey()) {
			if (isSelectable()) {
				/*
				 * If selectable we plagiate MSW behaviour: first scroll to the end of current view. If at the end, scroll down
				 * one page length and keep the selected row in the bottom part of visible area.
				 */
				// TODO raffael
				...
				// }
				// PMTableWidgetRow toBeFocusedRow = scrollBody.getRowByRowIndex(indexOfToBeFocused);
				//
		...
		if (keycode == getNavigationPageUpKey()) {
			if (isSelectable()) {
				...
				// PMTableWidgetRow firstVisibleRowInViewPort =
		...
		if (keycode == getNavigationStartKey()) {
			...
			if (isSelectable()) {
				if (focusedRow != null && focusedRow.getIndex() == 0) {
					...
					PMTableWidgetRow rowByRowIndex = (PMTableWidgetRow) scrollBody.iterator().next();
	...
	private boolean isFocusAtTheEndOfTable() {
		return focusedRow.getIndex() + 1 >= scrollBody.size();
	}

	private void scrollByPagelenght(int i) {
		int pixels = i * containerHeight;
	...
	public void onBlur(BlurEvent event) {
		...
		if (BrowserInfo.get().isIE()) {
			/*
			 * IE sometimes moves focus to a clicked table cell... (#7965) ...and sometimes it sends blur events even though
			 * the focus handler is still active. (#10464)
			 */
			Element focusedElement = Util.getIEFocusedElement();
			if (Util.getConnectorForElement(client, getParent(), focusedElement) == this && focusedElement != null
					&& focusedElement != scrollBodyPanel.getFocusElement()) {
				/*
				 * Steal focus back to the focus handler if it was moved to some other part of the table. Avoid stealing focus
				 * in other cases.
				 */
				focus();
				return;
	 ...
	 *          The key to remove
	 */
	private void removeRowFromUnsentSelectionRanges(PMTableWidgetRow row) {
	 ...
	 * <p>
	 * If the component has no explicit tabIndex a zero is given (default tabbing order based on dom hierarchy) or -1 if
	 * the component does not need to gain focus. The component needs no focus if it has no scrollabars (not scrollable)
	 * and not selectable. Note that in the future shortcut actions may need focus.
	 * <p>
	 * For internal use only. May be removed or replaced in the future.
	 */
	...
	private boolean isNavigationKey(int keyCode) {
		return keyCode == getNavigationUpKey() || keyCode == getNavigationLeftKey() || keyCode == getNavigationRightKey()
				|| keyCode == getNavigationDownKey() || keyCode == getNavigationPageUpKey()
				|| keyCode == getNavigationPageDownKey() || keyCode == getNavigationEndKey()
				|| keyCode == getNavigationStartKey();
	}

	public void lazyRevertFocusToRow(final PMTableWidgetRow currentlyFocusedRow) {
	 ...
	 * ("onselectstart",applyDisableTextSelectionIEHack()); Remove it then again when the mouse is depressed in the mouse
	 * up event.
	 * 
